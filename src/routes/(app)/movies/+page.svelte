<script lang="ts">
	...
	async function loadMovies() {
		...
		try {
			...
			const movieView = (await loadSupportedUserViews(api, userId)).find(
				(view) => view.type === 'movies'
			);
			if (!movieView) {
				items = [];
				return;
			...
			const responses = await Promise.all(
				movieView.libraryIds.map((parentId) =>
			...
			error = reason instanceof Error ? reason.message : 'Movies could not be loaded.';
		} finally {
			loading = false;
		}
	}
...
<svelte:head><title>Movies · Shayfin</title></svelte:head>
...
<div class="mx-auto w-full max-w-7xl space-y-6">
		...
		{#if !loading && !error}
			<p class="text-sm text-muted-foreground">
				{cards.length === items.length
					? `${items.length} titles`
					: `${cards.length} of ${items.length} titles`}
			</p>
		{/if}
	</header>
	...
	{#if error}
		<Alert variant="destructive">
			<AlertTitle>Movies are unavailable</AlertTitle>
			<AlertDescription class="flex flex-wrap items-center justify-between gap-3">
				<span>{error}</span>
				<Button variant="outline" size="sm" onclick={loadMovies}>
		...
		<div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
			...
			<div class="flex flex-col gap-3 sm:flex-row sm:items-center">
				...
				<Select.Root type="single" value={sort} onValueChange={chooseSort}>
					<Select.Trigger class="w-full sm:w-44" aria-label="Sort movies">
	...
	{:else if !error && cards.length === 0}
		<Empty.Root class="border border-border">
			<Empty.Header>
				<Empty.Media variant="icon"><FilmIcon /></Empty.Media>
				<Empty.Title>{items.length ? 'No matching movies' : 'No movie libraries found'}</Empty.Title
				>
				<Empty.Description>
					{items.length
